...
impl LlamaModel {
    ...
    pub async fn generate_with_ids(
        ...
        let mut generated_ids = input_ids.clone();
        println!("[OLD] Initial Token IDs: {:?}", &generated_ids);
        ...
        if prompt_len > 0 {
            ...
            println!(
                "[OLD] Prefill hidden state mean: {}",
                last_hidden.mean().unwrap()
            );
            println!("[OLD] Prefill hidden state std:  {}", last_hidden.std(0.0));
            let logits = self.lm_head.dot(&last_hidden);
        ...
        for i in 1..max_new_tokens {
            ...
            let generation_mask = full_attention_mask.slice(s![.., 0..current_len]).to_owned();
            ...
            let old_argmax = logits
                ...
                .0;
            println!(
                "[OLD] First logits argmax: {} ({})",
                old_argmax,
                self.tokenizer()
                    .decode(&[old_argmax as u32], false)
                    .unwrap()
            );
            let next_token = self.sample_token(&logits, temperature, top_k)?;
