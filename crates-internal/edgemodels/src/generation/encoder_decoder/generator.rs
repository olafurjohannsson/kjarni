use crate::generation::encoder_decoder::cpu_backend::{self, CpuBackend};
use crate::generation::encoder_decoder::gpu_backend::{self, GpuBackend};
use anyhow::{Result, anyhow};
use async_stream::try_stream;
use async_trait::async_trait;
use edgetransformers::cache::Cache;
use edgetransformers::common::StreamedToken;
use edgetransformers::encoder_decoder::traits::{
    EncoderDecoderGenerationBackend, EncoderDecoderLanguageModel,
};
use edgetransformers::encoder_decoder::{run_beam_search, run_beam_search_stream};
use edgetransformers::models::base::{DecodingStrategy, GenerationConfig, LanguageModel};
        ...
        match self {
            AnyEncoderDecoderBackend::Cpu(b) => {
                let tokens = decoder_tokens
                    .downcast_ref::<cpu_backend::CpuTensor>()
            ...
            AnyEncoderDecoderBackend::Gpu(b) => {
                let tokens = decoder_tokens
                    .downcast_ref::<gpu_backend::GpuSeq2SeqTensor>()
        ...
        match self {
            AnyEncoderDecoderBackend::Cpu(b) => {
                let concrete_tensor = tensor
                    .downcast_mut::<cpu_backend::CpuTensor>()
                    .ok_or_else(|| anyhow!("Mismatched Tensor type for CpuBackend"))?;
                b.update_token_tensor(concrete_tensor, new_tokens)
            }
            AnyEncoderDecoderBackend::Gpu(b) => {
                let concrete_tensor = tensor
                    .downcast_mut::<gpu_backend::GpuSeq2SeqTensor>()
                    .ok_or_else(|| anyhow!("Mismatched Tensor type for GpuBackend"))?;
                b.update_token_tensor(concrete_tensor, new_tokens)
            }
        }
