use anyhow::Result;
use ndarray::{Array2, Array3, Array4, Axis, s, Zip}; // Import Zip
use crate::linear_layer::LinearLayer;
use crate::utils::linear_algebra::{matmul_4d, matmul_4d_decode, matmul_4d_context};
use rayon::prelude::*;
...
impl DecoderSelfAttention {
    pub fn new(
        hidden_size: usize, num_heads: usize,
    ...
    pub fn forward(
        ...
        // 1. Flatten & Project
        let hidden_2d = hidden_states.view().into_shape((batch * seq_len, hidden_size))?;
        ...
        let k_heads = full_k.view()
            .into_shape((batch, total_seq, self.num_heads, self.head_dim))?
        ...
        let output_3d = output.into_shape((batch, seq_len, hidden_size))?;
...
impl DecoderCrossAttention {
    ...
    pub fn precompute_encoder_kv(&self, encoder_hidden_states: &Array3<f32>) -> Result<(Array4<f32>, Array4<f32>)> {
        let (batch, seq_len, _) = encoder_hidden_states.dim();
        let enc_2d = encoder_hidden_states.view()
    ...
    pub fn forward(
        ...
        let hidden_2d = hidden_states.view().into_shape((batch * seq_len, self.num_heads * self.head_dim))?;
        let q = self.q_proj.matmul(&hidden_2d);
        ...
        let output = self.o_proj.matmul(&context_flat.view());
        let output_3d = output.into_shape((batch, seq_len, self.num_heads * self.head_dim))?;
