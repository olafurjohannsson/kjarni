...
mod tests {
    ...
    async fn test_attention_q_projection_correctness() -> Result<()> {
        ...
        let gpu_q_permuted_array =
            read_buffer_3d(&context, &q_permuted_gpu, (num_heads, seq_len, head_dim))
                .await?;
    ...
    async fn test_attention_scores_correctness2() -> Result<()> {
        ...
        let gpu_scores_array = read_buffer_3d(&context, &scores_gpu, (num_heads, seq_len, seq_len)).await?;
