...
use anyhow::Result;
use ndarray::{s, Array, Array2, Axis, Array4};
...
async fn test_softmax_padded_case() -> Result<()> {
    ...
    cpu_data.slice_mut(s![.., 0..logical_cols]).assign(&valid_part);
...
async fn test_softmax_padded_case2() -> Result<()> {
    ...
    let valid_part = Array::random((batch, heads, query_len, logical_cols), Uniform::new(-1.0, 5.0));
    cpu_data_padded.slice_mut(s![.., .., .., 0..logical_cols]).assign(&valid_part);
    ...
    cpu_result_padded.slice_mut(s![.., .., .., 0..logical_cols]).assign(&cpu_result_valid);
