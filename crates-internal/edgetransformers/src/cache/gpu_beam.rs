...
use crate::gpu_ops::{GpuTensor, blocks::cache::reorder::GpuReorderCache, blocks::cache::GpuUpdateCache};
...
impl GpuBeamKVCache {
    ...
    pub fn new(
        ...
        if capacity == 0 { return Err(anyhow!("Cache capacity cannot be zero.")); }
        ...
        for i in 0..num_layers {
            k_tensors.push(GpuTensor::uninitialized(context, cache_shape.clone(), crate::gpu_ops::DType::F32, &format!("Layer {} K-Cache", i)));
            v_tensors.push(GpuTensor::uninitialized(context, cache_shape.clone(), crate::gpu_ops::DType::F32, &format!("Layer {} V-Cache", i)));
            // Allocate the temporary buffers for reordering
            temp_k_tensors.push(GpuTensor::uninitialized(context, cache_shape.clone(), crate::gpu_ops::DType::F32, &format!("Temp Layer {} K-Cache", i)));
            temp_v_tensors.push(GpuTensor::uninitialized(context, cache_shape.clone(), crate::gpu_ops::DType::F32, &format!("Temp Layer {} V-Cache", i)));
        }
    ...
    pub fn update(
        ...
        self.update_kernel.encode(encoder, new_k, new_v, cache_k, cache_v, position_offset);
        Ok(())
    }
    ...
    pub fn reorder(&mut self, encoder: &mut CommandEncoder, parent_indices: &GpuTensor) {
        ...
        for i in 0..self.k_tensors.len() {
            self.reorder_kernel.encode(encoder, &self.k_tensors[i], &self.temp_k_tensors[i], parent_indices, self.seq_length);
        }
        // Reorder V tensors
        for i in 0..self.v_tensors.len() {
            self.reorder_kernel.encode(encoder, &self.v_tensors[i], &self.temp_v_tensors[i], parent_indices, self.seq_length);
        }
