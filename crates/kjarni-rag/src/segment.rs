...
impl SegmentBuilder {
    ...
    pub fn new(temp_dir: &Path, dimension: usize, max_docs: usize) -> Result<Self> {
        ...
        let docs_writer = BufWriter::with_capacity(64 * 1024, File::create(&docs_path)?);

        let metadata_writer = BufWriter::with_capacity(32 * 1024, File::create(&metadata_path)?);
    ...
    pub fn add(
        ...
        let meta_json =
            serde_json::to_string(&metadata.unwrap_or(&std::collections::HashMap::new()))?;
...
impl Segment {
    ...
    pub fn open(path: &Path) -> Result<Self> {
        let meta: SegmentMeta =
            serde_json::from_str(&fs::read_to_string(path.join("segment.json"))?)?;
        ...
        let doc_offsets: Vec<u64> = bincode::deserialize(&fs::read(path.join("docs.idx"))?)?;

        // Load BM25 index
        let bm25: Bm25Index = bincode::deserialize(&fs::read(path.join("bm25.bin"))?)?;
    ...
    pub fn get_embedding(&self, doc_id: usize) -> Option<&[f32]> {
        ...
        let floats = unsafe {
            std::slice::from_raw_parts(bytes.as_ptr() as *const f32, self.meta.dimension)
        };
...
mod tests {
    ...
    fn test_segment_roundtrip() {
        ...
        builder
            .add("hello world", &[1.0, 0.0, 0.0, 0.0], None)
            .unwrap();
        builder
            .add("goodbye world", &[0.0, 1.0, 0.0, 0.0], None)
            .unwrap();
    ...
    fn test_bm25_search() {
        ...
        builder
            .add("rust programming language", &[1.0, 0.0, 0.0, 0.0], None)
            .unwrap();
        builder
            .add("python scripting", &[0.0, 1.0, 0.0, 0.0], None)
            .unwrap();
        builder
            .add("rust is fast", &[0.5, 0.5, 0.0, 0.0], None)
            .unwrap();
