...
impl StdFeedForwardNew {
    ...
    pub fn forward_noalloc(&self, hidden: &ArrayView2<f32>, buffers: &mut EncoderBuffers) {
        ...
        self.fc2
            .matmul_noalloc(&intermediate_view, &mut buffers.ffn_output);
    }
}
...
mod feedforward_new_tests {
    ...
    fn test_ffn_golden_values_gelu() -> Result<()> {
        let input_data = vec![0.5, -0.2000, 0.1000, -0.5, 0.0, 0.8000];
        let input = Array3::from_shape_vec((1, 2, 3), input_data)?;

        let w1_data = vec![
            0.4414, 0.4792, -0.1353, 0.5304, -0.1265, 0.1165, -0.2811, 0.3391, 0.509, -0.4236,
            0.5018, 0.1081,
        ];
        let fc1 = mock_linear(w1_data, (4, 3));

        let w2_data = vec![
            0.3694, 0.0677, 0.2411, -0.0706, 0.3854, 0.0739, -0.2334, 0.1274, -0.2304, -0.0586,
            -0.2031, 0.3317,
        ];
        ...
        // Expected Output
        let expected_data = vec![0.0266, 0.0386, -0.0491, 0.0304, -0.1196, 0.0148];
