...
mod seq2seq_encoder_tests {
    ...
    use crate::{
        traits::{
            ...
            EncoderLayout, EncoderLayerLayout,
            AttentionLayout, FeedForwardLayout
        },
    };
    use anyhow::Result;
        ...
        fn layout(&self) -> ModelLayout {
            ModelLayout {
                ...
                encoder: Some(EncoderLayout {
                    ...
                    position_embedding: if self.no_pos_emb_in_layout { None } else { Some("pos_emb".to_string()) },
                    ...
                    layer: EncoderLayerLayout {
                        self_attn: AttentionLayout {
                            ...
                            q_weight: "l0.q.weight".to_string(), q_bias: None,
                            k_weight: "l0.k.weight".to_string(), k_bias: None,
                            v_weight: "l0.v.weight".to_string(), v_bias: None,
                            o_weight: "l0.o.weight".to_string(), o_bias: None,
                            norm_weight: "l0.attn_ln.weight".to_string(),
                            norm_bias: Some("l0.attn_ln.bias".to_string()),
                        },
                        ffn: FeedForwardLayout {
                            up_weight: "l0.fc1.weight".to_string(), up_bias: None,
                            down_weight: "l0.fc2.weight".to_string(), down_bias: None,
    ...
    fn create_model_weights(weights_map: HashMap<String, Vec<f32>>, shapes: HashMap<String, Vec<usize>>) -> Result<(ModelWeights, TempDir)> {
        ...
        for (k, shape, bytes) in &stored_data {
            tensors.insert(k.clone(), TensorView::new(Dtype::F32, shape.clone(), bytes)?);
        }
    ...
    fn get_bart_golden_data() -> (HashMap<String, Vec<f32>>, HashMap<String, Vec<usize>>) {
        ...
        w.insert("pos_emb".into(), pos_data); s.insert("pos_emb".into(), vec![1024, 4]);

        w.insert("embed_ln.weight".into(), vec![1.0; 4]); s.insert("embed_ln.weight".into(), vec![4]);
        w.insert("embed_ln.bias".into(), vec![0.01; 4]); s.insert("embed_ln.bias".into(), vec![4]);
        w.insert("final_ln.weight".into(), vec![1.0; 4]); s.insert("final_ln.weight".into(), vec![4]);
        w.insert("final_ln.bias".into(), vec![0.01; 4]); s.insert("final_ln.bias".into(), vec![4]);

        // FIX: Keys updated to include .weight
        w.insert("l0.q.weight".into(), vec![
        ...
        w.insert("l0.attn_ln.weight".into(), vec![1.0; 4]); s.insert("l0.attn_ln.weight".into(), vec![4]);
        w.insert("l0.attn_ln.bias".into(), vec![0.01; 4]); s.insert("l0.attn_ln.bias".into(), vec![4]);
        w.insert("l0.ffn_ln.weight".into(), vec![1.0; 4]); s.insert("l0.ffn_ln.weight".into(), vec![4]);
        w.insert("l0.ffn_ln.bias".into(), vec![0.01; 4]); s.insert("l0.ffn_ln.bias".into(), vec![4]);

        let start = 4.201;
        let fc1_data: Vec<f32> = (0..32).map(|i| start + (i as f32 * 0.001)).collect();
        w.insert("l0.fc1.weight".into(), fc1_data); s.insert("l0.fc1.weight".into(), vec![8, 4]);

        let start_fc2 = start + 0.032;
        let fc2_data: Vec<f32> = (0..32).map(|i| start_fc2 + (i as f32 * 0.001)).collect();
        w.insert("l0.fc2.weight".into(), fc2_data); s.insert("l0.fc2.weight".into(), vec![4, 8]);
    ...
    fn get_whisper_golden_data() -> (HashMap<String, Vec<f32>>, HashMap<String, Vec<usize>>) {
        ...
        w.insert("token_emb".into(), vec![0.0; 40]); s.insert("token_emb".into(), vec![10, 4]);
        ...
        w.insert("l0.q.weight".into(), vec![
            ...
            0.049, 0.050, 0.051, 0.052, 0.053, 0.054, 0.055, 0.056,
        ]); s.insert("l0.q.weight".into(), vec![4, 4]);

        w.insert("l0.k.weight".into(), vec![
            0.057, 0.058, 0.059, 0.060, 0.061, 0.062, 0.063, 0.064,
            0.065, 0.066, 0.067, 0.068, 0.069, 0.070, 0.071, 0.072,
        ]); s.insert("l0.k.weight".into(), vec![4, 4]);
        ...
        w.insert("l0.o.weight".into(), vec![
            0.089, 0.090, 0.091, 0.092, 0.093, 0.094, 0.095, 0.096,
            0.097, 0.098, 0.099, 0.100, 0.101, 0.102, 0.103, 0.104,
        ]); s.insert("l0.o.weight".into(), vec![4, 4]);
    ...
    fn get_t5_golden_data() -> (HashMap<String, Vec<f32>>, HashMap<String, Vec<usize>>) {
        ...
        w.insert("encoder.block.0.layer.0.SelfAttention.relative_attention_bias.weight".into(), rel_data);
        s.insert("encoder.block.0.layer.0.SelfAttention.relative_attention_bias.weight".into(), vec![32, 2]);
    ...
    fn test_scenario_a_bart_postnorm_learned() -> Result<()> {
        ...
        let encoder = Seq2SeqCPUEncoder::new(&model_weights, &config, enc_config, ModelLoadConfig::default())?;
        ...
        let golden_data = vec![
            -1.331634, -0.437211, 0.457210, 1.351635, 
            -1.331635, -0.437211, 0.457214, 1.351632,
    ...
    fn test_scenario_b_whisper_prenorm_sinusoidal() -> Result<()> {
        ...
        let input_hidden = Array3::from_shape_vec((1, 3, 4), vec![
        ...
        let golden_data = vec![
            -1.153330, 0.814141, -0.794141, 1.173330, 
            0.247473, -0.264928, -1.361826, 1.419281,
    ...
    fn test_scenario_c_t5_prenorm_rel_bias() -> Result<()> {
        ...
        let golden_data = vec![
            -1.331581, -0.437194, 0.457194, 1.351581, 
            -1.331581, -0.437193, 0.457194, 1.351580,
