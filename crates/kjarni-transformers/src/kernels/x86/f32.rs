...
/// Computes the fused gate and up projections for a SwiGLU FFN with F32 weights.
///
/// This is the F32 equivalent of the fused BF16 SwiGLU kernel.
///
/// # Safety
///
/// This function is `unsafe` for the same reasons as `matmul_vec_f32`.
/// The caller must ensure CPU features are present and all slices are valid.
#[target_feature(enable = "avx2", enable = "fma")]
pub(crate) unsafe fn swiglu_fused_gate_up_f32(
    gate_out: &mut [f32],
    up_out: &mut [f32],
    a: &[f32],
    gate_w: &[f32],
    ...
    while j + 8 <= k {
        ...
        let gate_w_vec = _mm256_loadu_ps(gate_w_row.as_ptr().add(j));
        let up_w_vec = _mm256_loadu_ps(up_w_row.as_ptr().add(j));

        gate_sum_vec = _mm256_fmadd_ps(a_vec, gate_w_vec, gate_sum_vec);
        up_sum_vec = _mm256_fmadd_ps(a_vec, up_w_vec, up_sum_vec);
    ...
    while j < k {
        let a_val = a[j];
        gate_sum += a_val * gate_w_row[j];
        up_sum += a_val * up_w_row[j];
        j += 1;
    }
