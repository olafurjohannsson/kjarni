...
use kjarni_transformers::{LMHeadConfig, LoadedLMHead};
...
pub struct LlamaModel {
    pub decoder: Option<LlamaCpuDecoder>,
    pub gpu_decoder: Option<LlamaGpuDecoder>,
    tokenizer: Tokenizer,
    pub config: Arc<LlamaConfig>,
    
    lm_head: LinearLayer,
    gpu_lm_head_layer: Option<GpuLinearLayer>,
    gpu_lm_head_transposed: Option<GpuTensor>,

    lm_head_new: Option<LoadedLMHead>,
...
impl LlamaModel {
    ...
    pub fn from_pretrained(
        ...
        let load_cpu_head = device == Device::Cpu || load_config.offload_lm_head;
        let load_gpu_head = device == Device::Wgpu && !load_config.offload_lm_head;
        
        let lm_head_new = LoadedLMHead::new(
            context.as_ref(),
            &weights,
