...
fn default_rope_theta() -> f32 { 500000.0 }
    ...
    pub fn from_loader(loader: &dyn WeightLoader, config_json: Option<&str>) -> Result<Arc<Self>> {
        if loader.has_metadata() {
            ...
            let rope_scaling = rope_type.map(|rtype| RopeScalingConfig {
                ...
                original_max_position_embeddings: get_u32("rope.scaling.orig_ctx_len").map(|v| v as usize).unwrap_or(8192),
            });
            ...
            Ok(Arc::new(Self {
                ...
                num_key_value_heads: get_u32("attention.head_count_kv").unwrap_or(n_heads as u32 / 4) as usize,
                intermediate_size: get_u32("feed_forward_length").unwrap_or(hidden_size as u32 * 4) as usize,
