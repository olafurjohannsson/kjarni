mod bart_tests_config {
    ...
    use kjarni_transformers::{
        Device, LanguageModel, ModelType,
        common::{DecodingStrategy, GenerationConfig},
        encoder_decoder::{EncoderDecoderGenerator, EncoderDecoderLanguageModel},
        traits::ModelConfig,
    };
    ...
    async fn load_bart_large_cnn() -> Result<BartModel> {
        BartModel::from_registry(ModelType::BartLargeCnn, None, Device::Cpu, None, None).await
    }
    ...
    async fn test_bart_large_cnn_greedy_generation_news() -> Result<()> {
        let model =
            BartModel::from_registry(ModelType::BartLargeCnn, None, Device::Cpu, None, None)
                .await?;
    ...
    async fn test_bart_large_cnn_architectural_properties() -> Result<()> {
        ...
        assert_eq!(
            model.forced_bos_token_id(),
            Some(0),
            "forced_bos_token_id should be 0"
        );
        assert_eq!(
            model.forced_eos_token_id(),
            Some(2),
