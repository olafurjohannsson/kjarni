...
use kjarni_transformers::weights::{GgufLoader, SafeTensorsLoader, WeightLoader};
...
fn main() -> Result<()> {
    ...
    // let mut tensor_names: Vec<&str> = gguf_loader.tensor_names().into_iter().map(String::from).collect();
    // tensor_names.sort();
