...
use kjarni_rag::{LoaderConfig, SplitterConfig};
...
impl IndexerBuilder {
    ...
    pub fn chunk_overlap(mut self, overlap: usize) -> Self { self.chunk_overlap = overlap; self }
    
    // File filtering
    pub fn extension(mut self, ext: &str) -> Self {
        self.extensions.push(ext.to_lowercase().trim_start_matches('.').to_string());
        self
    }
    ...
    pub fn recursive(mut self, r: bool) -> Self { self.recursive = r; self }
    pub fn include_hidden(mut self, h: bool) -> Self { self.include_hidden = h; self }
    pub fn max_file_size(mut self, bytes: usize) -> Self { self.max_file_size = Some(bytes); self }
    
    // Index config
    pub fn max_docs_per_segment(mut self, n: usize) -> Self { self.max_docs_per_segment = n; self }
