...
pub unsafe extern "C" fn kjarni_search_results_free(results: KjarniSearchResults) {
    crate::panic::guard("kjarni_search_results_free", (), || unsafe {
...
pub unsafe extern "C" fn kjarni_searcher_free(searcher: *mut KjarniSearcher) {
    crate::panic::guard("kjarni_searcher_free", (), || {
        if !searcher.is_null() {
            unsafe {
...
pub unsafe extern "C" fn kjarni_searcher_search_with_options(
    ...
    crate::panic::guard(
        "kjarni_searcher_search_with_options",
        KjarniErrorCode::Panic,
        || -> KjarniErrorCode {
            ...
            if !options.is_null() {
                ...
                if !opts.source_pattern.is_null()
                    && let Ok(s) = unsafe { CStr::from_ptr(opts.source_pattern) }.to_str()
                {
                    filter = filter.source(s);
                    has_filter = true;
                }

                if !opts.filter_key.is_null()
                    && !opts.filter_value.is_null()
                    && let (Ok(k), Ok(v)) = (
...
pub unsafe extern "C" fn kjarni_searcher_has_reranker(searcher: *const KjarniSearcher) -> bool {
    crate::panic::guard("kjarni_searcher_has_reranker", false, || -> bool {
        if searcher.is_null() {
            return false;
        }
...
pub unsafe extern "C" fn kjarni_searcher_default_mode(
    ...
    crate::panic::guard(
        "kjarni_searcher_default_mode",
        KjarniSearchMode::Hybrid,
        || -> KjarniSearchMode {
            if searcher.is_null() {
...
pub unsafe extern "C" fn kjarni_searcher_default_top_k(searcher: *const KjarniSearcher) -> usize {
    crate::panic::guard("kjarni_searcher_default_top_k", 0, || -> usize {
        if searcher.is_null() {
            return 10;
        }
...
pub unsafe extern "C" fn kjarni_searcher_model_name(
    ...
    crate::panic::guard("kjarni_searcher_model_name", 0, || -> usize {
        unsafe {
            if searcher.is_null() {
                return 0;
            }
...
pub unsafe extern "C" fn kjarni_searcher_reranker_model(
    ...
    crate::panic::guard("kjarni_searcher_reranker_model", 0, || -> usize {
        unsafe {
            if searcher.is_null() {
                return 0;
            }
